...
from ..multiplayer import (answerRollCall, deleteGame, getGameByID, getGameIDWithPlayer,
...
class GameProtocol(Protocol):
    ...
    async def reqmove(self, ticket: str, *args):
        ...
        if ticket == DOUBLE_TICKET:
            moveData = move(self.game_id, self.player_id, DOUBLE_TICKET, {"ticket1": args[0], "location1": int(args[1]), "ticket2": args[2], "location2": int(args[3])})
        elif ticket == "pass":
            moveData = move(self.game_id, self.player_id, ticket, {})
        else:           
            moveData = move(self.game_id, self.player_id, ticket, {"location": int(args[0])})
