...
from . import __version__, multiplayer
from .engine.constants import GameState
from .engine.map import MAP
from .patterns import NAME_RE_PATTERN
...
def index(request: HttpRequest, game_id=None, error=None):
    ...
    if request.method == "POST":  # (create and) join game
        ...
        if not search(NAME_RE_PATTERN, player_name):
    ...
    return render(request, "scotlandyardclient/index.html", context=context)
...
def lobby(request: HttpRequest):
    ...
    res = render(request, "scotlandyardclient/lobby.html")
...
def game(request: HttpRequest):
    ...
    return render(request, "scotlandyardclient/game.html", context=context)
