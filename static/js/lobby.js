...
socket.onmessage = (msg) => {
  ...
  if (key === "ACKNOWLEDGE") {
    ...
    if (!los.includes(tokens[1])) los.push(tokens[1]);
  } else if (key === "DISCONNECT") {
    players.splice(
      players.findIndex((p) => p.player_id === tokens[1]),
      1
    );
...
function updateUI() {
  ...
  players.forEach((player) => {
    html += `<div class="row">
                <div class="col player${
                  los.includes(player.player_id) ? " los" : ""
                }" id="p-${
      player.player_id
    ...
    if (i !== -1) avail_colors.splice(i, 1);
  });
  ...
  if (my.isHost)
    players.forEach((player) => {
      ...
      if (los.includes(player.player_id)) btn.disabled = true;
...
document
  ...
  .setAttribute("d", circlePath(25, 30, 18));
